...
// دالة محلية لترحيل قيد يدعم المبلغ الأجنبي (نفس التعريف المستخدَم في Purchases.php وproducts.php —
// هذا الملف لا يُحمَّل أبداً معهما في نفس الطلب، فلا تعارض تسمية).
        ...
        try {
            ...
            if ($returns_discounts > 0) {
                ...
                if ($payable_acc && $discount_acc) {
                    ...
                    $rd_amount_syp = $returns_discounts * $rd_rate;
                    insertJournalLine($conn, $payable_acc, $rd_amount_syp, 0, $rd_entry_num, $today, $rd_desc, 'Manual', 'USD', $rd_rate, $returns_discounts, 0);
                    insertJournalLine($conn, $discount_acc, 0, $rd_amount_syp, $rd_entry_num, $today, $rd_desc, 'Manual', 'USD', $rd_rate, 0, $returns_discounts);
                }
            }
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['edit_supplier'])) {
    ...
    try {
        ...
        // تصحيح جوهري: تعديل هذا الحقل كان يُغيِّر "صافي حساب المورد" المعروض في هذه الصفحة وصفحة
        // الملف التفصيلي فوراً، بلا أي قيد محاسبي مقابل — بخلاف آلية "خصم مسجَّل" الأخرى (supplier_discounts)
        // التي تُرحِّل قيداً حقيقياً. هذا كان يجعل "ذمم الموردين" في القوائم المالية الرسمية (المبنية من
        // القيود فقط) غير متطابقة أبداً مع صفحة الموردين كلما استُخدِم هذا الحقل. الآن يُرحَّل الفرق
        // (delta) كقيد محاسبي حقيقي عند كل تغيير، بنفس منطق "خصومات مكتسبة من الموردين".
        $returns_delta = $returns_discounts - $old_returns;
        if (abs($returns_delta) > 0.00001) {
            $today = date('Y-m-d');
            $rd_rate = getExchangeRateForDate($conn, 'USD', $today);
            ...
            if ($payable_acc && $discount_acc) {
                ...
                // زيادة الخصم = يُخفِّض الذمة أكثر (مثل خصم جديد)؛ نقصانه = يُعيد جزءاً من الذمة (عكس خصم سابق)
                if ($returns_delta > 0) {
                    insertJournalLine($conn, $payable_acc, $rd_amount_syp, 0, $rd_entry_num, $today, $rd_desc, 'Manual', 'USD', $rd_rate, abs($returns_delta), 0);
                    insertJournalLine($conn, $discount_acc, 0, $rd_amount_syp, $rd_entry_num, $today, $rd_desc, 'Manual', 'USD', $rd_rate, 0, abs($returns_delta));
                } else {
                    insertJournalLine($conn, $discount_acc, $rd_amount_syp, 0, $rd_entry_num, $today, $rd_desc, 'Manual', 'USD', $rd_rate, abs($returns_delta), 0);
                    insertJournalLine($conn, $payable_acc, 0, $rd_amount_syp, $rd_entry_num, $today, $rd_desc, 'Manual', 'USD', $rd_rate, 0, abs($returns_delta));
                }
            }
        }
